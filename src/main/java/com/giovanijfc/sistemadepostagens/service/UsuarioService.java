...
public class UsuarioService {
	...
	public Usuario add(UsuarioDTO usuarioDto) {
		return new Usuario(null, usuarioDto.getNome(), usuarioDto.getEmail(), pe.encode(usuarioDto.getSenha()),
				new Date(System.currentTimeMillis()), pe.encode(usuarioDto.getPalavraChave()));
	}
	...
	public Usuario adicionarAmigo(Integer idUsuarioSecundario, Integer idUsuarioPrincipal) {
		...
		List<Amizade> listaAmizades2 = usuarioRepo.findById(idUsuarioSecundario).get().getAmizade().stream()
				.filter(x -> x.getUsuarioSecundario().getId() == userSecundario.getId()).collect(Collectors.toList());
		Amizade amizade = new Amizade(null, userPrincipal, userSecundario,new Date(System.currentTimeMillis()));
		Amizade amizade2 = new Amizade(null, userSecundario, userPrincipal,new Date(System.currentTimeMillis()));
